...
def save_md(results, file_path="readme.md"):
    ...
    with open(file_path, "w", encoding="utf-8") as f:
        ...
        latest = (
            results.get("win_stable_x64", {}).get("version")
            or results.get("win_stable_x86", {}).get("version")
            or results.get("win_stable_arm64", {}).get("version")
            or "Unknown"
        )

        f.write("# Google Chrome 离线安装包（请使用 7-Zip 解压）\n")
        f.write(f"> 📝 Auto update Chrome {latest} at {now}\n\n")
        f.write("稳定版存档：[Chrome_installer](https://github.com/wuyangdaily/chrome_installer/releases)\n\n")
        ...
        for channel in channel_order:
            ...
            f.write("| 架构 | 版本号 | 文件大小 | SHA256 | 下载 |\n")
            f.write("|------|--------|----------|--------|------|\n")
            for arch in arch_order:
                if arch in channels[channel]:
                    i = channels[channel][arch]
                    sha = i["sha256"][:16] + "..."
                    f.write(
                        f"| {arch} | `{i['version']}` | {i['size']} | `{sha}` | [下载]({i['url']}) |\n"
                    )
            f.write("\n")

            f.write("<details>\n")
            f.write("<summary>Full SHA-256</summary>\n\n")
            f.write("```\n")
            for arch in arch_order:
                if arch in channels[channel]:
                    info = channels[channel][arch]
                    asset = f"{arch}_{info['version']}"
...
def export_latest_version(results):
    latest = (
        results.get("win_stable_x64", {}).get("version")
        or results.get("win_stable_x86", {}).get("version")
        or results.get("win_stable_arm64", {}).get("version")
